...
function Deals({ instanceId }) {
  const classes = useStyles();
  const dispatch = useDispatch();
  const activeStep = useSelector((state) => state.solution.activeStep);
  // const mappedDealsStatus = useSelector(
  //   (state) => state?.solution?.mappedDealsStatus
  // );
  const loader = useSelector((state) => state.mapLoader.mapLoader);
  const selectedTab = useSelector(
    (state) => state?.solution?.mapFieldsSelectedTab
  );
  const initialRenderDeals = useSelector(
    (state) => state?.solution?.initialRenderDeals
  );
  const fetchedDealsList = useSelector(
    (state) => state?.solution?.fetchedDealsList
  );
  ...
  useEffect(() => {
    // axios({
    //   method: 'get',
    //   url: '/api/dataMapping/getDealsList',
    //   params: {
    //     instanceID:instanceId
    //   }
    // }).then((response)=> console.log(response));
    if (initialRenderDeals) {
      dispatch(actions.fetchMapDealsData(instanceId));
    }
    //dispatch(actions.fetchMapContactData(instanceId));
  }, []);

  useEffect(() => {
    if (fetchedDealsList.length) {
      setDisplayFilterData(fetchedDealsList);
      setDealsList(fetchedDealsList);
    }
  }, [fetchedDealsList]);
  //console.log("company list", dealsList);
  ...
  const swapValueHandler = () => {
    //alert(JSON.stringify(dealsList[id].sfCompanyName))
    const updatedList = displayFilterData;
    let fValue = displayFilterData[firstIndexVal].hsDealName;
    let sValue = displayFilterData[secondIndex].hsDealName;
    updatedList[firstIndexVal].hsDealName = sValue;
    updatedList[secondIndex].hsDealName = fValue;
    // console.log(
    ...
    dispatch(actions.removeDealsMapFieldFlag(false));
  ...
  const filteredResults = (results) => {
    const newResult = dealsList.filter((item) =>
  ...
  const dispatchStoreApi = () => {
    ...
    // axios({
    //     method: 'post',
    //     url: '/api/dataMapping/storeDealMappedData',
    //     data: {data:obj}
    //   }).then((response)=> console.log(response));

    dispatch(actions.mappedDealsData(obj));
  };
  const handleNext = () => {
  ...
  const handleRefreshUserList = () => {
    dispatch(actions.refreshDealsData(instanceId));
  };
  const handleResetUserList = () => {
    setDisplayFilterData(dealsList);
    dispatch(actions.fetchMapDealsData(instanceId));
    dispatch(actions.removeDealsMapFieldFlag(true));
  };

  // console.log("comapny list", dealsList);
  //confirm modal
  ...
  const handleDelete = (id) => {
    ...
    setDisplayFilterData(updatedList);
    setDealsList(updatedList);
    dispatch(actions.removeDealsMapFieldRow(id));
  };
  //end delete
 ...
 {dealsList.length ? <NotifySection /> : '' }
      {/* table bottom footer */}
      ...
      {/* {JSON.stringify(dealsList, null, 2)} */}
