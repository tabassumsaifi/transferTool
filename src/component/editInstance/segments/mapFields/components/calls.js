...
function Calls() {
    ...
    const initialRenderContact = useSelector(state=>state?.solution?.initialRenderContact) 
    const fetchedContactList = useSelector(state=>state?.solution?.fetchedContactList)    
...
//   console.log("fetchedContactList", fetchedContactList)
 ...
 useEffect(() => {
  ...
  useEffect(() => {
    if (fetchedContactList.length) setCompaniesList(fetchedContactList);
  }, [fetchedContactList]);
  ...
  const swapValueHandler = () => {
    ...
    let fValue = companiesList[firstIndexVal].hsContactName;
    let sValue = companiesList[secondIndex].hsContactName;
...
//   };
// search component
const searchableKeys = ["sfContactName"];
...
const handleNext = () => {
    ...
    //   }).then((response)=> console.log(response));
     ...
     dispatch(actions.mappedContactData(companiesList))
    dispatch(actions.setMapFieldsSelectedTabs(selectedTab + 1))
  }

  const handleRefreshUserList = () => {    
    dispatch(actions.refreshContactData("111122223333444"));
  };
