...
function Leads() {
  ...
  const initialRenderLead = useSelector(
    (state) => state?.solution?.initialRenderLead
  );
  const fetchedLeadList = useSelector(
    (state) => state?.solution?.fetchedLeadList
  );
  const [leadList, setLeadList] = useState([]);
  const [firstIndexVal, setFirstIndexVal] = useState();
  const [secondIndex, setSecondIndex] = useState();
  //   console.log("fetchedLeadList", fetchedLeadList)
  //API call
  useEffect(() => {
    // axios({
    //   method: 'get',
    //   url: '/api/dataMapping/getLeadsList',
    //   params: {
    //     instanceID:"111122223333444"
    //   }
    // }).then((response)=> console.log(response));
    if (initialRenderLead) {
      dispatch(actions.fetchMapLeadData("111122223333444"));
    }
    //dispatch(actions.fetchMapMeetingData("111122223333444"));
  }, []);

  useEffect(() => {
    if (fetchedLeadList.length) setLeadList(fetchedLeadList);
  }, [fetchedLeadList]);
  //console.log("company list", leadList);
  ...
  const swapValueHandler = () => {
    //alert(JSON.stringify(leadList[id].sfCompanyName))
    const updatedList = leadList;
    let fValue = leadList[firstIndexVal].hsContactID;
    let sValue = leadList[secondIndex].hsContactID;
    updatedList[firstIndexVal].hsContactID = sValue;
    updatedList[secondIndex].hsContactID = fValue;
    //console.log(leadList[firstIndexVal], leadList[secondIndex]);
  ...
  //   };
  // search component
  const searchableKeys = ["hsContactID"];
  const [results, setResults] = useState("");
  const filteredResults = leadList.filter((item) =>
  ...
  const handleNext = () => {
    ...
    dispatch(actions.mappedLeadData(obj));
    dispatch(actions.setMapFieldsSelectedTabs(selectedTab + 1));
  };

  const handleRefreshUserList = () => {
    dispatch(actions.refreshLeadData("111122223333444"));
  };
  ...
  //console.log('comapny list', leadList)
  ...
  return (
    <>
      ...
      {JSON.stringify(leadList, null, 2)}
