...
function Emails() {
    ...
    const initialRenderEmail = useSelector(state=>state?.solution?.initialRenderEmail) 
    const fetchedEmailList = useSelector(state=>state?.solution?.fetchedEmailList)    
    const [emailList, setEmailList] = useState([]);
    const [firstIndexVal, setFirstIndexVal] = useState();
  const [secondIndex, setSecondIndex] = useState();
//   console.log("fetchedEmailList", fetchedEmailList)
 //API call
 useEffect(() => {
    // axios({
    //   method: 'get',
    ...
    if(initialRenderEmail){  
        dispatch(actions.fetchMapEmailData("111122223333444"));
    }
    //dispatch(actions.fetchMapEmailData("111122223333444"));
  ...
  useEffect(() => {
    if (fetchedEmailList.length) setEmailList(fetchedEmailList);
  }, [fetchedEmailList]);
 // console.log("company list", emailList);
  ...
  const swapValueHandler = () => {
    //alert(JSON.stringify(emailList[id].sfCompanyName))
    const updatedList = emailList;
    let fValue = emailList[firstIndexVal].hsEmail;
    let sValue = emailList[secondIndex].hsEmail;
    updatedList[firstIndexVal].hsEmail = sValue;
    updatedList[secondIndex].hsEmail = fValue;
   // console.log(emailList[firstIndexVal], emailList[secondIndex]);
...
//   };
// search component
const searchableKeys = ["hsEmail"];
    const [results, setResults] = useState("");
    const filteredResults = emailList.filter((item) =>
...
const handleNext = () => {
  ...
  const handleRefreshUserList = () => {    
    dispatch(actions.refreshEmailData("111122223333444"));
  };
...
//console.log('comapny list', emailList)
