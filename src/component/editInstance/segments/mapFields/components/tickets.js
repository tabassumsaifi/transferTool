...
function Tickets({ instanceId }) {
  ...
  const initialRenderTicket = useSelector(
    (state) => state?.solution?.initialRenderTicket
  );
  const fetchedTicketList = useSelector(
    (state) => state?.solution?.fetchedTicketList
  );
  ...
  const searchableKeys = ["hsTicketName"];
  const [results, setResults] = useState("");
  const Externalids = useSelector((state)=>state.solution?.externalIdsList)
  const screenName = "casesMap";
  const [externalId, setExternalId] = useState("");
  //   console.log("fetchedTicketList", fetchedTicketList)
  const removeMapField = useSelector(
    (state) => state.solution?.ticketMapFieldFlag
  );
  ...
  useEffect(() => {
    // axios({
    //   method: 'get',
    //   url: '/api/dataMapping/getTicketsList',
    //   params: {
    //     instanceID:instanceId
    //   }
    // }).then((response)=> console.log(response));
    if (initialRenderTicket) {
      dispatch(actions.fetchMapTicketData(instanceId));
    }
    //dispatch(actions.fetchMapTicketData(instanceId));
  }, []);

  useEffect(() => {
    if (fetchedTicketList.length) {
      setDisplayFilterData(fetchedTicketList);
      setTicketList(fetchedTicketList);
    }
  }, [fetchedTicketList]);
  //console.log("company list", ticketList);
  ...
  const swapValueHandler = () => {
    //alert(JSON.stringify(ticketList[id].sfCompanyName))
    const updatedList = displayFilterData;
    let fValue = displayFilterData[firstIndexVal].hsTicketName;
    let sValue = displayFilterData[secondIndex].hsTicketName;
    updatedList[firstIndexVal].hsTicketName = sValue;
    updatedList[secondIndex].hsTicketName = fValue;
    // console.log(
    ...
    dispatch(actions.removeTicketMapFieldFlag(false));
  ...
  const filteredResults = (results) => {
    const newResult = ticketList.filter((item) =>
  ...
  const dispatchStoreApi = () => {
    ...
    dispatch(actions.mappedTicketData(obj));
  };
  const handleNext = () => {
  ...
  const handleRefreshUserList = () => {
    dispatch(actions.refreshTicketData(instanceId));
  };
  const handleResetUserList = () => {
    setDisplayFilterData(ticketList);
    dispatch(actions.fetchMapTicketData(instanceId));
    dispatch(actions.removeTicketMapFieldFlag(true));
  };

  //console.log("comapny list", ticketList);
  ...
  //console.log("externalId", externalId);
   ...
   const handleDelete = (id) => {
    ...
    dispatch(actions.removeTicketMapFieldRow(id));
  };
  //end delete
    ...
    <>
      ...
      {ticketList.length ? (
        ...
        ""
           ...
           {ticketList.length ?  <NotifySection /> : ''}
      {/* table bottom footer */}
